...
RUN /bin/sh -c '$JBOSS_HOME/bin/standalone.sh &' && \
    ...
    for i in {1..120}; do if [ -f "$JBOSS_HOME/standalone/log/server.log" ] && grep -q "WildFly Full .* started in" "$JBOSS_HOME/standalone/log/server.log"; then break; else sleep 1; fi; done && \
